...
const ClassManagement = () => {
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      const { data, error } = await classesService.getByCourse(selectedCourse.id);
      if (!error) setClassDetail(data ?? []);
            ...
            <div className="p-2 space-y-1">
              {courses.map(course => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl transition-all text-sm ${selectedCourse?.id === course.id
